...
const articles = [
    ...
    function serve(req, res) {
        var parts = req.url.split('/'); // URL zerlegen
        var input = parts[1];
        var aId = parts[2];
    
        if (input === 'articles') {
            if (aId) {
                // Wandelt die Artikel-ID in eine Zahl um und prüft ob sie gültig ist
                var articleIndex = parseInt(aId) - 1;
                if (articleIndex >= 0 && articleIndex < articles.length) {
                    ...
                    // Wandelt die Artikel-Daten in JSON-Format um und sendet sie
                    res.end(JSON.stringify(articles[articleIndex], null, 2));
                } else {
                    res.writeHead(404, {
                        'Content-Type': 'text/plain',
                        'Access-Control-Allow-Origin': '*'
                    });
                    res.end('Artikel nicht gefunden\n');
                }
            } else {
                res.writeHead(200, {
                    'Content-Type': 'application/json',
